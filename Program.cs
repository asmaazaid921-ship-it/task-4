...
class Program
{
    static void Main()
    {
        ...
        AccountUtil.Deposit(new List<Account>(accounts), 1000);
        AccountUtil.Withdraw(new List<Account>(accounts), 2000);
        ...
        AccountUtil.Deposit(new List<Account>(savAccounts), 1000);
        ...
        AccountUtil.Withdraw(new List<Account>(trustAccounts), 500);
    }
        ...
        public static void Deposit(List<Account> accounts, double amount)
        {
            ...
            foreach (var acc in accounts)
            {
                if (acc.Deposit(amount))
                    Console.WriteLine($"Deposited {amount} to {acc.Name}, Balance = {acc.Balance}");
                else
        ...
        public static void Withdraw(List<Account> accounts, double amount)
        {
            ...
            foreach (var acc in accounts)
            {
                if (acc.Withdraw(amount))
                    Console.WriteLine($"Withdrew {amount} from {acc.Name}, Balance = {acc.Balance}");
                else
